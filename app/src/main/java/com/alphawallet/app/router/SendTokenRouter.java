...
public class SendTokenRouter {
    ...
    public void openTx(Activity context, String address, String txAddress, float amount, String symbol, int decimals, Wallet wallet, long chainId) {
        Intent intent = new Intent(context, SendActivity.class);
        intent.putExtra(C.EXTRA_CONTRACT_ADDRESS, address);
        intent.putExtra(C.EXTRA_ADDRESS, address);
        intent.putExtra("TX_ADDRESS", txAddress);
