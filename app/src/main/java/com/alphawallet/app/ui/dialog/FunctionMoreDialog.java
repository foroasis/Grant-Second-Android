...
public class FunctionMoreDialog extends BaseBottomSheetDialog {
    ...
    private void loadSize() {
        Utilities.globalQueue.postRunnable(() -> {
            ...
            long cacheEmojiSize = getDirectorySize(FileLoader.checkDirectory(FileLoader.MEDIA_DIR_CACHE), 3);
            stickersCacheSize += cacheEmojiSize;
            long audioSize = getDirectorySize(FileLoader.checkDirectory(FileLoader.MEDIA_DIR_AUDIO), 0);
            long totalSize = cacheSize + cacheTempSize + videoSize + audioSize + photoSize + documentsSize + musicSize + stickersCacheSize;
...
//            long totalSize = cacheSize + videoSize + audioSize + photoSize + documentsSize + musicSize + stickersSize;
            AndroidUtilities.runOnUIThread(() -> {
                if (totalSize > 1024 * 1024 * 5) { // 5M以内不显示
