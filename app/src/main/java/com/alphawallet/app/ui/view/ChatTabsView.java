...
public class ChatTabsView extends FrameLayout {
    ...
    public void initTabs() {
        tabs.clear();
        tabs.add(new Tab(-1, R.drawable.chat_tab_silence_off));
    ...
    public void addUserChainIcon(WalletInfo walletInfo) {
        if (!CollectionUtils.isEmpty(walletInfo.getChain_record())) {
            ...
            if (chainTabs.size() > 0) {
                ...
                chainRv.setLayoutManager(new LinearLayoutManager(context, LinearLayoutManager.HORIZONTAL, false));
                ...
                ListAdapter chainAdapter = new ListAdapter(context, true);
                ...
                if (chainTabs.size() > 4) {
                    chainContainer.addView(chainRv, LayoutHelper.createFrame(120, LayoutHelper.WRAP_CONTENT, Gravity.CENTER, 0, 0, 5, 0));
                } else {
                    chainContainer.addView(chainRv, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.CENTER, 0, 0, 5, 0));
                }
            }
        }
    }
    ...
    public void updateTabs() {
        for (int i = 0; i < tabs.size(); i++) {
            Tab tab = tabs.get(i);
            if (tab.id == -1) {
                tab.icon = chatActivity.getMessagesController().silentModeIds.get(chatActivity.getDialogId())!=null ?
                        R.drawable.chat_tab_silence_off : R.drawable.chat_tab_silence_on;
            } else if (tab.id == 0) {
                tab.icon = chatActivity.getMessagesController().isDialogMuted(chatActivity.getDialogId(), chatActivity.getTopicId()) ?
                        R.drawable.chat_tab_mute_off : R.drawable.chat_tab_mute_on;
            }
        }
    ...
    public class TabView extends FrameLayout {
        ...
        public TabView(Context context) {
            ...
            addView(imageView, LayoutHelper.createFrame(22, 22, Gravity.CENTER));
        }
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
        ...
        private boolean ifWarpContent;

        public ListAdapter(Context context, boolean ifWarpContent) {
            mContext = context;
            this.ifWarpContent = ifWarpContent;
        }
