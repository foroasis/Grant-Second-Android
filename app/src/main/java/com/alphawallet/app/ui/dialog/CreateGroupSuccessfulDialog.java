...
public class CreateGroupSuccessfulDialog extends Dialog {
    ...
    private void initView() {
        ...
        //邀请连接标题
        String shareTitle = LocaleController.getString("group_details_invitation_title", R.string.group_details_invitation_title);
        binding.tvGroupInvitationTitle.setText(shareTitle);
        //链接
        ...
        binding.ivInvitationQrcode.setOnClickListener(v -> {
            if (qrCodeBottomSheet == null) {
                qrCodeBottomSheet = new QRCodeBottomSheet(getContext(), shareTitle, shareUrl, LocaleController.getString("QRCodeLinkHelpGroup", R.string.QRCodeLinkHelpGroup), true) {
                    @Override
                    public void dismiss() {
                        super.dismiss();
