...
public class UserPersonalView extends FrameLayout implements View.OnClickListener, OnItemClickListener {
    ...
    private void initView() {
        ...
        binding.tvWalletTitle.setText(LocaleController.getString("dialog_user_personal_wallettoken", R.string.dialog_user_personal_wallettoken));
        binding.tvTransferTo.setText(LocaleController.getString("dialog_user_personal_transfer", R.string.dialog_user_personal_transfer));
        binding.tvInviteCreate.setText(LocaleController.getString("dialog_user_personal_invite_create", R.string.dialog_user_personal_invite_create));
        binding.tvSendRedPacketTo.setText(LocaleController.getString("dialog_user_personal_redpacket", R.string.dialog_user_personal_redpacket));
        ...
        //点击跳转到对方钱包页面
        binding.hsvChain.setOnClickListener(v -> {
        ...
        //邀请他人绑定钱包
        binding.tvInviteCreate.setOnClickListener(v -> {
            ...
            args.putInt("operationType", ChatActivity.opera_invite_bind_wallet);
        ...
        binding.tvTransferTo.setOnClickListener(v -> {
            ...
            args.putInt("operationType", ChatActivity.opera_transfer);
            ChatActivity chatActivity = new ChatActivity(args);
            fragment.presentFragment(chatActivity);
            EventBus.getDefault().post(new MessageEvent(EventBusTags.DISMISS_DIALOG));
        });
        ...
        //点击弹出发红包弹窗
        binding.tvSendRedPacketTo.setOnClickListener(v -> {
            Bundle args = new Bundle();
            args.putLong("user_id", user.id);
            if (!fragment.getMessagesController().checkCanOpenChat(args, fragment)) {
                return;
            }
            args.putInt("operationType", ChatActivity.opera_send_packet);
            ChatActivity chatActivity = new ChatActivity(args);
            fragment.presentFragment(chatActivity);
            EventBus.getDefault().post(new MessageEvent(EventBusTags.DISMISS_DIALOG));
        });
...
//                }
    ...
    private void openUrl(String url, Browser.Progress progress) {
        if (url.startsWith("@")) {
            fragment.getMessagesController().openByUserName(url.substring(1), fragment, 0, progress);
     ...
     * 未绑定钱包样式
     */
    private void unBindWalletUi() {
        binding.hsvChain.setVisibility(GONE);
        binding.ivWalletInfoArrow.setVisibility(GONE);
        binding.tvInviteCreate.setVisibility(VISIBLE);

        binding.tvTransferTo.setClickable(false);
        binding.tvSendRedPacketTo.setClickable(false);

        binding.tvTransferTo.setAlpha(0.2f);
        binding.tvSendRedPacketTo.setAlpha(0.2f);
    }

    /***
     * 绑定钱包样式
     */
    private void bindWalletUi() {
        binding.tvInviteCreate.setVisibility(GONE);
        binding.hsvChain.setVisibility(VISIBLE);
        binding.ivWalletInfoArrow.setVisibility(VISIBLE);

        binding.tvTransferTo.setClickable(true);
        binding.tvSendRedPacketTo.setClickable(true);

        binding.tvTransferTo.setAlpha(1f);
        binding.tvSendRedPacketTo.setAlpha(1f);
    }
    ...
    public void setWalletData(WalletInfo walletInfo) {
        ...
        binding.llChain.removeAllViews();
        List<Integer> chainIconList = new ArrayList<>();
        for (WalletInfo.ChainInfoItem chainInfoItem : walletInfo.getChain_record()) {
            ...
            chainIconList.add(resID);
        }

        for (Integer resId : chainIconList) {
            ImageView chainIcon = new ImageView(getContext());
            chainIcon.setImageResource(resId);
            binding.llChain.addView(chainIcon, LayoutHelper.createLinear(20, 20, 0, 0, 11, 0));
        }
    }
}