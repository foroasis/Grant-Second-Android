...
public class CreateGroupView extends LinearLayout {
    ...
    //钱包配置数据
    private Web3ConfigEntity webConfigData;
    ...
    public CreateGroupView(BaseFragment baseFragment) {
    ...
    private void setThemeColor() {
        ...
        mBinding.etInputTokenId.setBackgroundColor(baseFragment.getThemedColor(Theme.key_windowBackgroundWhite));
        ...
        mBinding.tvTokenIdTitle.setTextColor(baseFragment.getThemedColor(Theme.key_windowBackgroundWhiteBlackText));
        mBinding.etInputTokenId.setHintTextColor(baseFragment.getThemedColor(Theme.key_windowBackgroundWhiteHintText));
        mBinding.etInputTokenId.setTextColor(baseFragment.getThemedColor(Theme.key_windowBackgroundWhiteBlackText));
    ...
    private void initClick() {
        ...
        //去绑定钱包页面
        mBinding.tvUnbindWallet.setOnClickListener(v -> new WalletUnbindDialog(getContext()).show());
    }
    ...
    private void initView() {
        ...
        //私人群组类型
        mBinding.tvGroupTypeTitle.setText(LocaleController.getString("create_group_type", R.string.create_group_type));
        ...
        mBinding.etInputTokenId.setHint(LocaleController.getString("create_group_input_unlimited", R.string.create_group_input_unlimited));
    ...
    private void payLimitUI() {
        ...
        setWalletAddress(this::defultValueSet);
    }

    /**
     * 用户有没有绑定过钱包地址，判断显示
     */
    private void setWalletAddress(Runnable runnable) {
        ...
        if (wallet == null) {
            ...
            walletName = wallet.getConnectedWalletPkg();
            ...
            for (Web3ConfigEntity.WalletNetworkConfigEntityItem walletData : webConfigData.getWalletType()) {
                BlockchainConfig.WalletIconType walletIconType = BlockchainConfig.getWalletTypeByPkg(walletName);
                if (walletIconType != null) {
                    if (walletIconType.walletType.equals(walletData.getName())) {
                        //钱包id
            ...
            if (StringUtils.isEmpty(walletName)) {
                mCreateGroupApi.setWallet_name("Particle Network");
                mCreateGroupApi.setWallet_id(100);
            } else {
     ...
     * 链 token 币种默认选择
     */
    private List<Web3ConfigEntity.WalletNetworkConfigEntityItem> conditionTokenData = new ArrayList<>();
    private List<Web3ConfigEntity.WalletNetworkConfigEntityItem> payLimitTokenData = new ArrayList<>();
    ...
    private void defultValueSet() {
        ...
        for (Web3ConfigEntity.WalletNetworkConfigEntityItem tokenData : webConfigData.getTokenType()) {
     ...
     */
    private void setSelectChainDataUI(Web3ConfigEntity.WalletNetworkConfigChainType selectChainData) {
        //设置选中数据给对话框
     ...
     */
    private void setSelectTokenDataUI(Web3ConfigEntity.WalletNetworkConfigEntityItem selectTokenData) {
        //设置选中的token数据
        ...
        if (selectTokenData.getName().equals("ERC20")) { //币种协议布局
            ...
            mBinding.tvTokenTitle.setText(LocaleController.getString("create_group_token_address_title", R.string.create_group_token_address_title));
            mBinding.rlSelectorCoinType.setVisibility(GONE);
            mBinding.etInputCoinnumMin.setVisibility(GONE);
            mBinding.rlInputPaynum.setVisibility(GONE);
        ...
        mBinding.llTokenId.setVisibility(selectTokenData.getName().equals("ERC1155") ? VISIBLE : GONE);
    }
     ...
     */
    private void setSelectCoinDataUI(Web3ConfigEntity.WalletNetworkConfigEntityItem selectCoinData) {
        //设置选中的币种数据
        ...
        //币种图标
        GlideHelper.getDrawableGlide(mBinding.getRoot().getContext(), selectCoinData.getIcon(), drawable -> {
    ...
    private void obtionWebConfigData(Runnable runnable) {
        ...
        mBinding.etInputTokenId.setText("");

        if (webConfigData == null) {
            webConfigData = MMKVUtil.getWeb3ConfigData();
            runnable.run();
        } else {
            runnable.run();
        }
    ...
    public CreateGroupApi getmCreateGroupApi() {
        ...
        if (mCreateGroupApi.getJoin_type() == 3) {
            ...
            if (WalletUtil.isSolanaAddress(ethWallet.getAddress())) {
                if (mCreateGroupApi.getChain_id() != 99999) {
                    new WalletListDialog(baseFragment).show();
            ...
            } else if (WalletUtil.isEvmAddress(ethWallet.getAddress())) {
                if (mCreateGroupApi.getChain_id() == 99999) {
                    new WalletListDialog(baseFragment).show();
                    ToastUtils.showLong(LocaleController.getString("common_wallet_switch_solana_address", R.string.common_wallet_switch_solana_address));
                    return null;
                }
            }
        }
        ...
        if (mCreateGroupApi.getToken_name().equals("ERC20")) { //代币协议
            ...
            String tokenAddress = mBinding.etInputTokenAddress.getText().toString().trim();
            ...
            if (mCreateGroupApi.getToken_name().equals("ERC1155")) {
                String tokenId = mBinding.etInputTokenId.getText().toString();
                if (!StringUtils.isEmpty(tokenId)) {
                    mCreateGroupApi.setNft_token_id(tokenId);
                }
            }
        }
